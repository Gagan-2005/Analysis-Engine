...
def load(source: str, target_dir: Path, branch: str | None = None) -> RepositoryInfo:
    """
    Validates and shallow clones an HTTPS Git repository into the workspace.
    """
    ...
    logger.info(f"Initiating HTTPS clone for repository: {name} from {source}")

    commit_hash = None
    final_local_path = None
    is_successful = False
    
    validation = validate_repository_source(source, RepositoryType.HTTPS)
